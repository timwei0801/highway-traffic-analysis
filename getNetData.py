...
if __name__ == "__main__":
    ...
    data_types = [
        "VD", "ETag", "News", "Traffic", "Static", 
        "M03A", "M05A", "M06A", "M08A", "Jilian_ETag"
    ]
    
    # 1. 下載假日資料（主要資料集）
    weekend_dates = collector.get_weekend_dates(start_date, end_date)
    print(f"將下載 {len(weekend_dates)} 天的假日資料")
    weekend_results = collector.download_multiple_dates(weekend_dates, data_types)
    ...
    print(f"將下載 {len(weekday_dates)} 天的平日資料")
    weekday_results = collector.download_multiple_dates(weekday_dates, data_types)
    
    print(f"資料下載完成，共 {len(weekend_dates) + len(weekday_dates)} 天")
